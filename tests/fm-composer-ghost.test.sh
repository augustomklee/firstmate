#!/usr/bin/env bash
# Ghost-text robustness (incident composer-robust), fork edition.
#
...
#
# Fork note: the primitives live in bin/fm-tmux-lib.sh and call "$FM_MUX". We pin
# FM_MUX=tmux so the per-test fake `tmux` on PATH is exercised even on a machine
# where psmux is installed (which fm-mux-lib would otherwise prefer).
set -u

# Pin the multiplexer to `tmux` BEFORE sourcing the lib, so fm-mux-lib does not
# resolve a real psmux and the per-test fake tmux on PATH is used.
export FM_MUX=tmux
...
# A fake tmux that serves a styled composer line for the dim-aware reader.
# capture-pane returns the styled fixture verbatim WITH -e (mirrors
# `tmux capture-pane -e`), and the same content with SGR sequences stripped
# WITHOUT -e (mirrors a plain capture). cursor_y comes from FM_FAKE_CY.
make_fake_tmux() {  # <dir>
