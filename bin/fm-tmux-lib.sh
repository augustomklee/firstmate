#!/usr/bin/env bash
# fm-tmux-lib.sh — shared multiplexer pane primitives for firstmate.
#
...
#
# Fork note (psmux / Windows): every multiplexer call goes through "$FM_MUX"
# (resolved by fm-mux-lib.sh — psmux on Windows, tmux elsewhere), never a raw
# `tmux`. This file sources fm-mux-lib.sh itself so it is usable standalone.
#
...
# ANSI styling (capture-pane -e), drops dim/faint (SGR 2) runs, and decides on
...
#
# All functions are `set -u` and `set -e` safe (guarded multiplexer calls,
# explicit returns) so they can be sourced into either context.

# Resolve $FM_MUX (psmux/tmux) so this lib works standalone. Idempotent: a
# caller that already sourced fm-mux-lib.sh just re-sets FM_MUX to the same value.
# shellcheck source=bin/fm-mux-lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/fm-mux-lib.sh"
...
# styled line on stdin (from `$FM_MUX capture-pane -e`) and prints plain text on
...
#   unknown - the pane could not be read (multiplexer error). The caller decides.
#
...
# fm_tmux_submit_enter_core: send Enter to <target>, verifying the composer
# cleared. Retries Enter ONLY — never retypes, because a swallowed Enter leaves
# the text in the composer and retyping would duplicate it. Echoes the final
# verdict on stdout (empty|pending|unknown).
...
# verifying the composer cleared. Retries Enter ONLY — never retypes. Echoes the
# final verdict on stdout (empty|pending|unknown|send-failed) so callers can pick
# their own success policy:
...
fm_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  ...
  "$FM_MUX" send-keys -t "$target" -l "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
